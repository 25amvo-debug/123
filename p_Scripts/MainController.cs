...
public class MainController : MonoBehaviour
{
    ...
    public void getSlides()
    {
        ...
        if (dataStorage.isFirst = true || dataStorage.activeTheme != 1)
        {
            SendMessages("Ваша презентація готова", "Пора вчитись", colors[5]);
        }
    ...
    public void EndTestingYoohooo(float mark, float time, bool rept)
    {
        ...
        float[] data = { dataStorage.avarageMark, mark, time };
        float studSkill = FindAnyObjectByType<Network>().StartNetwork(data);
        if (!rept) { dataStorage.levels[dataStorage.activeTheme] = studSkill; dataStorage.activeTheme++; }
